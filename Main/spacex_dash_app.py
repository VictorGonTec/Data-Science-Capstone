...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                dcc.Dropdown(id='site-dropdown', options = options, value = 'All Sites', placeholder = "Select a Launch Site here", searchable = True),
                                html.Br(),
...
def scatter(val1, val2):
    ...
    if val1 == 'All Sites':
        new_df = spacex_df
        new_df2 = new_df[new_df["Payload Mass (kg)"] >= val2[0]]
        new_df3 = new_df2[new_df["Payload Mass (kg)"] <= val2[1]]
        fig2 = px.scatter(new_df3, y="class", x="Payload Mass (kg)", color="Booster Version Category")
    else:
        new_df = spacex_df[spacex_df["Launch Site"] == val1]
        new_df2 = new_df[new_df["Payload Mass (kg)"] >= val2[0]]
        new_df3 = new_df2[new_df["Payload Mass (kg)"] <= val1[1]]
        fig2 = px.scatter(new_df3, y="class", x="Payload Mass (kg)", color="Booster Version Category")
    return fig2
